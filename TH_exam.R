...
path = "~/Example/path/to/the/file/Data_MidTermExam_IntlFinanceV2.xlsx"

Q2a.FX_spot_rate <- read_excel(path, sheet = "Q2a. FX spot rate", range = "A2:C154")

Q2a.Bond_yields_3M <- read_excel(path, sheet = "Q2a. Bond rates", range = "A5:C157")

Q2a.Bond_yields_1Y <- read_excel(path, sheet = "Q2a. Bond rates", range = "F5:H157")

Q2a.Bond_yields_3Y <- read_excel(path, sheet = "Q2a. Bond rates", range = "K5:M157",
                                 col_types = c("text", "numeric", "numeric"))

Q2a.Bond_yields_5Y <- read_excel(path, sheet = "Q2a. Bond rates", range = "P5:R157",
                                 col_types = c("text", "numeric", "numeric"))

Q2a.Bond_yields_10Y <- read_excel(path, sheet = "Q2a. Bond rates", range = "U5:W157")

Q2b.Spot_rates <- read_excel(path, sheet = "Q2b. Spot rates")
Q2b.Spot_rates <- Q2b.Spot_rates %>%
  rename(Date = me)

Q2b.Forward_rates <- read_excel(path, sheet = "Q2b. Forward rates", col_types = c("date", 
...
# Regressions
lm_reg.3M <- lm(formula = `annual_delta_s_(t+k1)` ~ `i_min_i*`, data = reg.3M)
